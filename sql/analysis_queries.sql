-- ============================================================
-- Messi Legacy Analytics — SQL Analysis
-- Tables: club_stats, international_stats
-- ============================================================
...
SELECT
    ...
    total_goals,
    total_assists,
    ROUND(1.0 * total_goals / NULLIF(total_appearances, 0), 2) AS goals_per_appearance,
    total_trophies
FROM club_totals
...
WITH per_season AS (
    SELECT
        ...
        goals,
        assists,
        ROUND(1.0 * (goals + assists) / NULLIF(appearances, 0), 2) AS ga_per_app
    FROM club_stats
)
SELECT
    season,
...
-- 6. International tournaments: trophy count and goals per tournament, best to worst
SELECT
    year,
    tournament,
    result,
    goals,
    assists,
    CASE WHEN result IN ('Champion') THEN 1 ELSE 0 END AS trophy_won
FROM international_stats
ORDER BY goals DESC;

-- 7. Career-long summary: club vs international goal totals (UNION-based comparison)
...
FROM international_stats WHERE result = 'Champion'
UNION ALL
SELECT
    CAST(SUBSTR(season, 1, 4) AS INTEGER) AS trophy_year,
    club AS competition,
    'Club' AS scope
FROM club_stats
...
--    (contradicts the standard athletic decline curve)
WITH totals AS (
    SELECT
...
--     season goal tallies, using SQLite's built-in aggregate functions
WITH barca AS (
    SELECT goals FROM club_stats WHERE club = 'FC Barcelona'
),
stats AS (
    SELECT
        AVG(goals) AS mean_goals,
        -- population stdev via variance formula (SQLite has no native STDEV)
        SQRT(AVG(goals * goals) - AVG(goals) * AVG(goals)) AS stdev_goals
    FROM barca
)
SELECT
    ROUND(mean_goals, 1) AS mean_goals_per_season,
    ROUND(stdev_goals, 1) AS stdev_goals_per_season,
    ROUND(stdev_goals / mean_goals, 2) AS coefficient_of_variation
FROM stats;

-- 11. Trophy efficiency: goals required per trophy won, club vs. international —
--     tests the "couldn't win for country" narrative against the numbers
SELECT 'FC Barcelona' AS scope, SUM(goals) AS goals, SUM(trophies_won) AS trophies,
       ROUND(1.0 * SUM(goals) / NULLIF(SUM(trophies_won), 0), 1) AS goals_per_trophy
FROM club_stats WHERE club = 'FC Barcelona'
UNION ALL
SELECT 'Paris Saint-Germain', SUM(goals), SUM(trophies_won),
       ROUND(1.0 * SUM(goals) / NULLIF(SUM(trophies_won), 0), 1)
FROM club_stats WHERE club = 'Paris Saint-Germain'
UNION ALL
SELECT 'Inter Miami CF', SUM(goals), SUM(trophies_won),
       ROUND(1.0 * SUM(goals) / NULLIF(SUM(trophies_won), 0), 1)
FROM club_stats WHERE club = 'Inter Miami CF'
UNION ALL
SELECT 'Argentina (International)', SUM(goals), SUM(CASE WHEN result = 'Champion' THEN 1 ELSE 0 END),
       ROUND(1.0 * SUM(goals) / NULLIF(SUM(CASE WHEN result = 'Champion' THEN 1 ELSE 0 END), 0), 1)
FROM international_stats
ORDER BY goals_per_trophy;

-- 12. Resilience: "dip-then-bounce-back" seasons, using LAG/LEAD window functions.
--     A dip is any season with fewer goals than the prior season, and a bounce-back
--     is a dip immediately followed by a season with more goals than the dip.
WITH ordered AS (
    SELECT
        season, club, goals,
...
-- 13. World Cup arc: goals per World Cup across all six tournaments, showing the
--     non-linear ("U-shaped") career arc — his two best World Cups by goals were
--     his last two, at ages 35 and 39
SELECT
    year,
    result,
    goals,
...
SELECT
    ...
    END AS career_phase,
    ROUND(AVG(1.0 * assists / NULLIF(goals + assists, 0)), 3) AS avg_assist_share
FROM club_stats
WHERE season IN ('2004-05','2005-06','2006-07','2007-08','2024','2025','2026')
GROUP BY career_phase;

-- 15. Best tournament performance in a losing campaign — foreshadowing the
--     trophy breakthrough that came years later
